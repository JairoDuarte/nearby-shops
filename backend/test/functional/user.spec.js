...

{
	...
	test('User add like shop', async ({ client, assert }) => {
		...
		const response = await client
			...
			.loginVia(user, 'jwt')
			.send({name:'Zara Casa',address:'Casablanca',photos:'imp.jpg',id:1222})
			.accept('json')
			.end()
	...
	test('User remove shop from preferred list', async ({ client, assert }) => {
		let user = await User.first()
		await user.likesShops().save(new Shop({name:'Zara Casa',address:'Casablanca',photos:'imp.jpg',id:1222}))
		await user.likesShops().save(new Shop({name:'Apple ',address:'Casablanca',photos:'imp.jpg',id:12215645642}))
